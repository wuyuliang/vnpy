...
对 `159915.SZ` 易方达创业板 ETF 在 `2024-07-01` 至 `2026-07-01` 期间进行
...
- 2024-07-01 开盘前资金为 `1,000,000` 元。
- 初始持仓为 0，最终状态冷却计时器为空。
- 区间前行情只用于 EMA、状态指标和 2024-06-28 收盘预测的预热。
- 不继承 2024-07-01 以前的仓位、盈亏或冷却状态。
- 2026-07-01 收盘按市值结算，不为报告目的强制平仓。
...
原 EMA 策略从 2024-07-01 空仓开始独立运行，产生每日
...
结果文档必须列出组合、EMA 和买入持有对比，逐年收益，最大回撤区间，换手、成本、仓位
分布、信号诊断及是否满足回撤/换手约束。
