...
def detect_breakout_pullback(
    ...
    for idx in anchor_idx:
        ...
        if i0 + 1 > end:
            continue
        atr_anchor = float(a.iloc[i0]) if np.isfinite(a.iloc[i0]) else np.nan
        if not np.isfinite(atr_anchor) or atr_anchor <= 0:
            continue

        # P0 fix: pb_extreme 必须只使用“到确认 bar 为止”的可见数据，
        # 不能扫描整段未来窗口后再回写到当前确认 bar。
        if direction == "long":
            for j in range(i0 + 1, end + 1):
                if not np.isfinite(close.iloc[j]) or float(close.iloc[j]) <= brk_level:
                    continue
                sl_conf = slice(i0 + 1, j + 1)
                pb_extreme = float(low.iloc[sl_conf].min())
                dist_atr = (brk_level - pb_extreme) / atr_anchor if np.isfinite(pb_extreme) else np.nan
            ...
            for j in range(i0 + 1, end + 1):
                if not np.isfinite(close.iloc[j]) or float(close.iloc[j]) >= brk_level:
                    continue
                sl_conf = slice(i0 + 1, j + 1)
                pb_extreme = float(high.iloc[sl_conf].max())
                dist_atr = (pb_extreme - brk_level) / atr_anchor if np.isfinite(pb_extreme) else np.nan
                valid = np.isfinite(dist_atr) and (0.0 <= dist_atr <= max_pullback_atr)
                if not valid:
                    continue
                out.at[out.index[j], "bp_valid"] = True
