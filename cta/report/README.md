...
- **不要往 `cta/backtest/<run>` 里手工添东西**：所有内容都应该可由原始 command 复现；手工产物（如人写的复盘 md）请放到 `change_log.md` 或 `cta/docs/review/`。
...
- **size 警告**：本目录会快速膨胀（每次 OOT 产物 50~500MB）。定期跑 cleanup 脚本前先用 `du -sh cta/backtest/*/` 看 top 10。
