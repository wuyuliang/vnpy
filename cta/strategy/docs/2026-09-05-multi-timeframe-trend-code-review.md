...
- **N-04 最终口径**：价格、R 标尺和允许回吐量均按建仓初始手数计算，部分减仓不再收紧地板。
...
价格/R 双向换算使用 `initial_quantity`，避免减仓重写历史峰值。最终实现按后续决议，
允许回吐量也保持建仓时口径，不再按剩余手数缩放。

### 必补测试

- 地板尚未启用前先减半，2R 峰值仍得到 1R 地板。
- 地板已启用后减半，地板不下降，后续允许回吐量保持不变。
- 多头、空头对称。
