...
logger = logging.getLogger("cta.model.oot.oot_intrabar")
...
class _IntrabarBarCache:
    ...
    def load(self, symbol: str, exchange: str) -> pd.DataFrame:
        ...
        try:
        ...
        except (FileNotFoundError, NotADirectoryError, PermissionError, OSError, KeyError, ValueError) as exc:
            logger.warning(
                "intrabar bars unavailable for %s_%s interval=%s: %s",
                key[0],
                key[1],
                self.interval,
                exc,
            ...
            raise RuntimeError(
                f"unexpected intrabar load error for {key[0]}_{key[1]} interval={self.interval}: {exc}"
            ) from exc
        if not bars.empty:
            bars = bars.copy()
