...
from cta.model.oot.pipeline_oot_evaluation_base import BR_BLOCKED_CLUSTER_CAP, BR_BLOCKED_DAILY_POSITION, BR_BLOCKED_FINAL_DECISION_GATE, BR_BLOCKED_LEVERAGE, BR_BLOCKED_MARGIN_CASH, BR_BLOCKED_MFE_MAE_GATE, BR_BLOCKED_MONTHLY_DRAWDOWN, BR_BLOCKED_PORTFOLIO_CONSTRAINT, BR_BLOCKED_REGIME_GATE, BR_BLOCKED_SYMBOL_CAP, BR_BLOCKED_TOTAL_CONCURRENT, BR_BLOCKED_TRADE_FILTER, BR_BLOCKED_WEEKLY_BUDGET, BR_BLOCKED_WEEKLY_DRAWDOWN, BR_ZERO_NOTIONAL
...
def _evaluate_oot_real_execution(prediction_df: pd.DataFrame, *, cfg: OotEvaluationConfig=DEFAULT_OOT_EVAL_CONFIG, intrabar_bar_provider: Callable[[str, str, pd.Timestamp, pd.Timestamp, str], pd.DataFrame] | None=None, extra_outputs: dict[str, pd.DataFrame] | None=None, htf_reference_df: pd.DataFrame | None=None) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    trade_ret_pct = np.maximum(gross_ret_pct, -intrabar_stop_pct_series); cost_pct = float(cfg.commission_pct_per_trade) + float(cfg.slippage_pct_per_trade); net_ret_pct = trade_ret_pct - cost_pct; selected['gross_return_pct'] = pd.to_numeric(gross_ret_pct, errors='coerce').fillna(0.0); selected['trade_return_pct'] = pd.to_numeric(net_ret_pct, errors='coerce').fillna(0.0); selected['net_return_pct'] = selected['trade_return_pct']; selected['cost_pct'] = cost_pct; selected['month'] = selected['exit_datetime'].dt.to_period('M').dt.to_timestamp(); selected = selected.sort_values(['entry_datetime', 'exit_datetime']).reset_index(drop=True); n = len(selected)
    ...
    pl_cfg = getattr(cfg, 'portfolio_logic', None); use_pl_runtime = bool(getattr(cfg, 'use_portfolio_logic_runtime', False)); use_pl_trailing = bool(use_pl_runtime and pl_cfg is not None and bool(getattr(pl_cfg, 'enable_trailing', False))); use_pl_htf = bool(use_pl_runtime and pl_cfg is not None and bool(getattr(pl_cfg, 'enable_htf_gate', False))); use_pl_ranker = bool(use_pl_runtime and pl_cfg is not None and bool(getattr(pl_cfg, 'enable_ranker', False))); use_pl_throttle = bool(use_pl_runtime and pl_cfg is not None and bool(getattr(pl_cfg, 'enable_risk_throttle', False))); use_pl_pyramid = bool(use_pl_runtime and pl_cfg is not None and bool(getattr(pl_cfg, 'enable_pyramid', False))); htf_gate: HtfGate | None = None; htf_ref_by_interval: dict[str, pd.DataFrame] = {}; htf_state_cache: dict[pd.Timestamp, dict[tuple[str, str], dict[str, Any]]] = {}
    if use_pl_htf:
        ...
        for itv in pl_cfg.interval_gate.htf_intervals:
            ref_interval = ref.get('interval', pd.Series([''] * len(ref), index=ref.index)); ref_interval_norm = ref_interval.map(normalize_portfolio_interval); part = ref.loc[ref_interval_norm == normalize_portfolio_interval(itv)].copy()
        ...
        if len(pred_intervals_norm) == 1 and ref_intervals_seen:
            pred_interval = next(iter(pred_intervals_norm)); pred_rank = float(interval_rank.get(pred_interval, 0.0)); semantic_intervals = [k for k in ref_intervals_seen if float(interval_rank.get(k, 0.0)) >= pred_rank]
            if set(semantic_intervals) != set(ref_intervals_seen):
                dropped = sorted(set(ref_intervals_seen) - set(semantic_intervals)); logger.info('HTF intervals filtered by rank for prediction interval=%s (rank=%.2f): kept=%s dropped=%s (HTF must be >= candidate interval rank, see Fix-E).', pred_interval, pred_rank, semantic_intervals, dropped)
                for k in dropped:
        ...
        if not ref_intervals_seen:
            logger.warning('HTF gate enabled but htf_reference contains no rows for any of %s; disabling HTF gate for this run (see cta/docs/block_reason.md §4).', pl_cfg.interval_gate.htf_intervals); use_pl_htf = False; htf_gate = None
        elif set(ref_intervals_seen) != set(configured_intervals):
            narrowed_cfg = dataclasses.replace(pl_cfg.interval_gate, htf_intervals=tuple(ref_intervals_seen)); htf_gate = HtfGate(narrowed_cfg); logger.info('HTF intervals narrowed from %s to %s (htf_reference only contains these).', configured_intervals, tuple(ref_intervals_seen))
    ...
    if use_intrabar:
        preferred_intrabar_raw = str(getattr(cfg, 'intrabar_tracking_interval', '60min')); preferred_intrabar = normalize_interval(preferred_intrabar_raw); fallback_raw = tuple(getattr(cfg, 'intrabar_tracking_fallback_intervals', ())); interval_candidates: list[str] = []
        ...
        for key, grp in selected.groupby([selected.get('symbol', pd.Series([''] * n, index=selected.index)).astype(str).str.upper(), selected.get('exchange', pd.Series([''] * n, index=selected.index)).astype(str).str.upper()]):
            entry_min = pd.to_datetime(grp['entry_datetime'], errors='coerce').min(); exit_max = pd.to_datetime(grp['exit_datetime'], errors='coerce').max()
        ...
        bar_cache_by_interval = {itv: _IntrabarBarCache(interval=itv, date_span_by_symbol=span_by_symbol) for itv in interval_candidates}
        ...
        for idx in range(n):
            ...
            if pd.isna(ent) or pd.isna(exi):
                continue
            sym = str(symbol_arr[idx]).upper(); ex = str(exchange_arr[idx]).upper(); hint = float(entry_price_arr[idx]) if np.isfinite(entry_price_arr[idx]) else float('nan'); used_interval = interval_candidates[0]
            if intrabar_bar_provider is not None:
                used_interval = preferred_intrabar_raw
                try:
                    bars = intrabar_bar_provider(sym, ex, pd.Timestamp(ent), pd.Timestamp(exi), used_interval)
                except Exception:
                    bars = pd.DataFrame()
            else:
                bars = pd.DataFrame()
                for itv in interval_candidates:
                    ...
                    if not bars_try.empty:
                        bars = bars_try; used_interval = itv; break
            if use_pl_trailing:
                row_interval = normalize_portfolio_interval(selected.iloc[idx].get('interval', '')); row_regime = str(selected.iloc[idx].get('pred_regime_label', '')).strip().lower(); atr_pct = float(atr_pct_arr[idx]) if idx < len(atr_pct_arr) and np.isfinite(atr_pct_arr[idx]) else None; _row_stop_pct = float(intrabar_stop_pct_arr[idx]) if idx < len(intrabar_stop_pct_arr) else _intrabar_default; hold_score = float(hold_extend_arr[idx]) if idx < len(hold_extend_arr) and np.isfinite(hold_extend_arr[idx]) else None; rec_ext = int(recommended_ext_arr[idx]) if idx < len(recommended_ext_arr) else None; sim = simulate_trailing_exit(side=str(side_arr[idx]), entry_ts=pd.Timestamp(ent), planned_exit_ts=pd.Timestamp(exi), entry_price_hint=hint, stop_loss_pct=_row_stop_pct, bars=bars, interval=row_interval, atr_pct_at_entry=atr_pct, regime_label=row_regime, cfg=pl_cfg.trailing, horizon_cfg=pl_cfg.horizon_extend if bool(getattr(pl_cfg, 'enable_horizon_extend', False)) else None, hold_extend_score=hold_score, recommended_extension_bars=rec_ext, taper_cfg=pl_cfg.oscillation_taper if bool(getattr(pl_cfg, 'enable_oscillation_taper', False)) else None, cluster=infer_symbol_cluster(str(symbol_arr[idx])))
            else:
                _row_stop_pct = float(intrabar_stop_pct_arr[idx]) if idx < len(intrabar_stop_pct_arr) else _intrabar_default; sim = _simulate_intrabar_exit(side=str(side_arr[idx]), entry_ts=pd.Timestamp(ent), planned_exit_ts=pd.Timestamp(exi), entry_price_hint=hint, stop_loss_pct=_row_stop_pct, bars=bars)
            selected.at[idx, 'entry_fill_datetime'] = sim['entry_fill_datetime']; selected.at[idx, 'entry_fill_price'] = sim['entry_fill_price']; selected.at[idx, 'planned_exit_datetime'] = sim['planned_exit_datetime']; selected.at[idx, 'planned_exit_price'] = sim['planned_exit_price']; selected.at[idx, 'stop_loss_price'] = sim['stop_loss_price']; selected.at[idx, 'stop_hit_datetime'] = sim['stop_hit_datetime']; selected.at[idx, 'stop_hit_price'] = sim['stop_hit_price']; selected.at[idx, 'stop_triggered'] = int(sim['stop_triggered']); selected.at[idx, 'final_exit_datetime'] = sim['final_exit_datetime']; selected.at[idx, 'final_exit_price'] = sim['final_exit_price']; selected.at[idx, 'stop_tracking_interval'] = used_interval; selected.at[idx, 'exit_reason'] = sim['exit_reason']; selected.at[idx, 'trailing_activated'] = int(sim.get('trailing_activated', 0)); selected.at[idx, 'trailing_stop_price'] = sim.get('trailing_stop_price', np.nan); selected.at[idx, 'extensions_used'] = int(sim.get('extensions_used', 0)); selected.at[idx, 'position_taper_count'] = int(sim.get('position_taper_count', 0)); selected.at[idx, 'position_taper_target_ratio'] = float(sim.get('position_taper_target_ratio', 1.0)); selected.at[idx, 'position_taper_realized_ratio'] = float(sim.get('position_taper_realized_ratio', 0.0)); ret = float(sim['price_return_pct']) if np.isfinite(sim['price_return_pct']) else float('nan')
            if np.isfinite(ret):
                _row_stop_pct = float(intrabar_stop_pct_arr[idx]) if idx < len(intrabar_stop_pct_arr) else _intrabar_default; g = max(ret, -_row_stop_pct); gross_pct_arr[idx] = float(g); net_pct_arr[idx] = float(g - cost_pct)
    ...
    cash = float(cfg.initial_capital); margin_used = 0.0; open_notional = 0.0; runtime_state = PortfolioState(equity=float(cash)); pre_blocked_mask = selected['execution_status'].astype(str) != 'pending'
    ...
    for ts in timeline:
        ...
        for idx in exits_map.get(ts, []):
            if idx not in open_positions:
                continue
            pos = open_positions.pop(idx); margin_used = max(0.0, float(margin_used - float(pos['margin']))); open_notional = max(0.0, float(open_notional - float(pos['notional']))); pos_sym = str(pos.get('symbol', '')); runtime_state.remove_position(str(pos.get('pos_id', '')))
            if pos_sym:
        ...
        if use_pl_ranker and ranker is not None and entry_order:
            ...
            for idx in entry_order:
                prob_raw = pd.to_numeric(pd.Series([selected.iloc[idx].get('trade_filter_prob', np.nan)]), errors='coerce').iloc[0]; prob_pctl = pd.to_numeric(pd.Series([selected.iloc[idx].get('trade_filter_prob_pctl', np.nan)]), errors='coerce').iloc[0]
                if not np.isfinite(prob_pctl):
                    prob_pctl = float(np.clip((float(prob_raw) if np.isfinite(prob_raw) else 0.0) * 100.0, 0.0, 100.0))
                cand_rows.append({'_row_idx': int(idx), 'symbol': str(symbol_arr[idx]).upper(), 'exchange': str(exchange_arr[idx]).upper(), 'interval': normalize_portfolio_interval(selected.iloc[idx].get('interval', '')), 'direction': str(side_arr[idx]).lower(), 'cluster_name': infer_symbol_cluster(str(symbol_arr[idx]).upper()), 'trade_filter_prob_pctl': float(prob_pctl), 'pred_mfe_atr': float(pd.to_numeric(pd.Series([selected.iloc[idx].get('pred_mfe_atr', np.nan)]), errors='coerce').iloc[0]), 'pred_mae_atr': float(pd.to_numeric(pd.Series([selected.iloc[idx].get('pred_mae_atr', np.nan)]), errors='coerce').iloc[0]), 'htf_alignment': str(selected.iloc[idx].get('htf_alignment', 'neutral') or 'neutral'), 'base_notional': float(cash) * float(cfg.max_position_scale)})
            cand_df = pd.DataFrame(cand_rows); scored = ranker.score(cand_df, htf_state={})
            for _, row in scored.iterrows():
                i = int(row['_row_idx']); selected.at[i, 'ranker_score'] = float(row.get('score', np.nan))
            state = PortfolioState.from_dict(runtime_state.to_dict()); state.equity = float(cash)
            caps_eff = pl_cfg.caps; min_prob_pctl_for_bar = 0.0
            if use_pl_throttle and risk_throttle is not None and (throttle_level is not None):
                caps_eff = risk_throttle.apply_to_caps(pl_cfg.caps, throttle_level); score_threshold_for_bar = max(float(pl_cfg.ranker.score_threshold_baseline), float(throttle_level.score_pctl_threshold) / 100.0); min_prob_pctl_for_bar = float(getattr(throttle_level, 'effective_min_prob_pctl', 0.0))
            else:
                score_threshold_for_bar = float(pl_cfg.ranker.score_threshold_baseline)
            picks = ranker.allocate(scored, state=state, caps=caps_eff, score_threshold=float(score_threshold_for_bar), min_prob_pctl=float(min_prob_pctl_for_bar)); selected_idx = {int(x) for x in picks.get('_row_idx', pd.Series([], dtype=int)).tolist()}
            for idx in entry_order:
                selected.at[idx, 'ranker_score_threshold'] = float(score_threshold_for_bar); selected.at[idx, 'ranker_prob_pctl_threshold'] = float(min_prob_pctl_for_bar)
                if int(idx) not in selected_idx:
                    selected.at[idx, 'execution_status'] = 'blocked_ranker'; selected.at[idx, 'block_reason'] = 'ranker_dropped'; selected.at[idx, 'equity_before'] = float(cash); selected.at[idx, 'equity_after'] = float(cash)
        ...
        for idx in entry_order:
            ...
            if bool(cfg.use_portfolio_constraints):
                if bool(cfg.block_new_entries_on_monthly_dd_breach) and month_dd_breached:
                    ...
                    cap_daily = max(0.0, float(day_start_equity * float(cfg.max_daily_new_notional_pct) - day_new_notional)); cap_lev = max(0.0, float(entry_equity * float(cfg.max_total_leverage) - float(runtime_state.total_open_notional))); cap_cash = max(0.0, float(avail_cash / margin_rate)); cap_symbol = max(0.0, float(entry_equity * float(cfg.max_symbol_notional_pct) - sym_notional_now)); cap_cluster = float('inf')
                    if use_pl_runtime and pl_cfg is not None:
                        cap_symbol = max(0.0, float(entry_equity * float(pl_cfg.caps.max_symbol_notional_pct) - sym_notional_now)); cap_cluster = max(0.0, float(entry_equity * float(pl_cfg.caps.max_cluster_notional_pct) - cluster_notional_now))
                    cap_week = float('inf')
                    if bool(cfg.enforce_weekly_dd_budget_on_entry) and float(cfg.weekly_max_drawdown_pct) > 0:
                        used_dd_amt = max(0.0, float(week_peak_equity - entry_equity)); remain_dd_amt = max(0.0, float(week_peak_equity * float(cfg.weekly_max_drawdown_pct) - used_dd_amt))
                    ...
                    if notional <= 0:
                        if cap_symbol <= 0:
                            ...
                            reason = BR_BLOCKED_PORTFOLIO_CONSTRAINT; logger.error('blocked_portfolio_constraint hit unexpectedly at idx=%d: notional=%.6f cap_daily=%.6f cap_lev=%.6f cap_cash=%.6f cap_week=%.6f cap_symbol=%.6f cap_cluster=%.6f', int(idx), float(notional), float(cap_daily), float(cap_lev), float(cap_cash), float(cap_week), float(cap_symbol), float(cap_cluster))
            ...
            pos_id_now = str(selected.at[idx, 'pos_id']); open_positions[idx] = {'notional': notional, 'margin': margin, 'gross_ret_pct': float(gross_pct_arr[idx]), 'net_ret_pct': float(net_pct_arr[idx]), 'entry_equity': entry_equity, 'symbol': sym_key, 'exchange': ex_key, 'side': side_now, 'cluster': cluster_key, 'pos_id': pos_id_now, 'layer_id': int(pd.to_numeric(pd.Series([selected.at[idx, 'layer_id']]), errors='coerce').fillna(0).iloc[0]), 'entry_ts': pd.Timestamp(ts)}; runtime_state.add_position({'pos_id': pos_id_now, 'symbol': sym_key, 'exchange': ex_key, 'cluster': cluster_key, 'direction': side_now, 'notional': float(notional)})
    for idx, pos in list(open_positions.items()):
        margin_used = max(0.0, float(margin_used - float(pos['margin']))); open_notional = max(0.0, float(open_notional - float(pos['notional']))); pos_sym = str(pos.get('symbol', '')); runtime_state.remove_position(str(pos.get('pos_id', '')))
        if pos_sym:
            symbol_open_notional[pos_sym] = max(0.0, float(symbol_open_notional.get(pos_sym, 0.0) - float(pos['notional']))); symbol_open_count[pos_sym] = max(0, int(symbol_open_count.get(pos_sym, 0) - 1))
        pos_cluster = str(pos.get('cluster', 'other')); cluster_open_notional[pos_cluster] = max(0.0, float(cluster_open_notional.get(pos_cluster, 0.0) - float(pos['notional']))); cluster_open_count[pos_cluster] = max(0, int(cluster_open_count.get(pos_cluster, 0) - 1))
        if use_pl_pyramid and idx in idx_layer_ref:
    ...
    open_positions.clear(); executed = selected.loc[selected['execution_status'] == 'executed'].copy()
    if executed.empty:
        summary = pd.DataFrame([{'oot_rows': oot_rows, 'executed_rows': executed_rows, 'trade_count': 0, 'selected_rows': selected_rows, 'blocked_rows': int((selected['execution_status'] != 'executed').sum()), 'blocked_trade_filter_rows': int((selected['execution_status'] == BR_BLOCKED_TRADE_FILTER).sum()), 'blocked_regime_gate_rows': int((selected['execution_status'] == BR_BLOCKED_REGIME_GATE).sum()), 'blocked_mfe_mae_rows': int((selected['execution_status'] == BR_BLOCKED_MFE_MAE_GATE).sum()), 'blocked_final_decision_rows': int((selected['execution_status'] == BR_BLOCKED_FINAL_DECISION_GATE).sum()), 'blocked_margin_cash_rows': int((selected['execution_status'] == 'blocked_margin_cash').sum()), 'blocked_leverage_rows': int((selected['execution_status'] == 'blocked_leverage').sum()), 'blocked_limit_move_rows': int((selected['execution_status'] == 'blocked_limit_move').sum()), 'blocked_daily_position_rows': int((selected['execution_status'] == 'blocked_daily_position').sum()), 'blocked_weekly_drawdown_rows': int((selected['execution_status'] == 'blocked_weekly_drawdown').sum()), 'blocked_weekly_budget_rows': int((selected['execution_status'] == 'blocked_weekly_budget').sum()), 'blocked_monthly_drawdown_rows': int((selected['execution_status'] == 'blocked_monthly_drawdown').sum()), 'blocked_symbol_cap_rows': int(selected['execution_status'].isin(['blocked_symbol_cap', 'blocked_cluster_cap']).sum()), 'blocked_symbol_concurrent_rows': int((selected['execution_status'] == 'blocked_symbol_concurrent').sum()), 'blocked_total_concurrent_rows': int((selected['execution_status'] == 'blocked_total_concurrent').sum()), 'blocked_htf_rows': int((selected['execution_status'] == 'blocked_htf_gate').sum()), 'blocked_ranker_rows': int((selected['execution_status'] == 'blocked_ranker').sum()), 'blocked_throttle_rows': int((selected['execution_status'] == 'blocked_throttle_halt').sum()), 'blocked_pyramid_rows': int((selected['execution_status'] == 'blocked_pyramid_rule').sum()), 'exit_datetime_fixup_rows': exit_fixup_rows, 'stop_loss_exit_rows': 0, 'trailing_stop_exit_rows': 0, 'horizon_exit_rows': 0, 'monthly_obs': 0, 'gross_pnl': 0.0, 'net_pnl': 0.0, 'roll_cost_total': 0.0, 'total_return_pct': 0.0, 'avg_monthly_return_pct': float('nan'), 'std_monthly_return_pct': float('nan'), 'monthly_excess_return_pct': float('nan'), 'std_monthly_excess_return_pct': float('nan'), 'monthly_sharpe': float('nan'), 'max_drawdown_pct': float('nan'), 'annualized_return_pct': float('nan'), 'calmar_like': float('nan')}])
    ...
    summary = pd.DataFrame([{'oot_rows': oot_rows, 'executed_rows': executed_rows, 'trade_count': int(len(executed)), 'selected_rows': selected_rows, 'blocked_rows': int((selected['execution_status'] != 'executed').sum()), 'blocked_trade_filter_rows': int((selected['execution_status'] == BR_BLOCKED_TRADE_FILTER).sum()), 'blocked_regime_gate_rows': int((selected['execution_status'] == BR_BLOCKED_REGIME_GATE).sum()), 'blocked_mfe_mae_rows': int((selected['execution_status'] == BR_BLOCKED_MFE_MAE_GATE).sum()), 'blocked_final_decision_rows': int((selected['execution_status'] == BR_BLOCKED_FINAL_DECISION_GATE).sum()), 'blocked_margin_cash_rows': int((selected['execution_status'] == 'blocked_margin_cash').sum()), 'blocked_leverage_rows': int((selected['execution_status'] == 'blocked_leverage').sum()), 'blocked_limit_move_rows': int((selected['execution_status'] == 'blocked_limit_move').sum()), 'blocked_daily_position_rows': int((selected['execution_status'] == 'blocked_daily_position').sum()), 'blocked_weekly_drawdown_rows': int((selected['execution_status'] == 'blocked_weekly_drawdown').sum()), 'blocked_weekly_budget_rows': int((selected['execution_status'] == 'blocked_weekly_budget').sum()), 'blocked_monthly_drawdown_rows': int((selected['execution_status'] == 'blocked_monthly_drawdown').sum()), 'blocked_symbol_cap_rows': int(selected['execution_status'].isin(['blocked_symbol_cap', 'blocked_cluster_cap']).sum()), 'blocked_symbol_concurrent_rows': int((selected['execution_status'] == 'blocked_symbol_concurrent').sum()), 'blocked_total_concurrent_rows': int((selected['execution_status'] == 'blocked_total_concurrent').sum()), 'blocked_htf_rows': int((selected['execution_status'] == 'blocked_htf_gate').sum()), 'blocked_ranker_rows': int((selected['execution_status'] == 'blocked_ranker').sum()), 'blocked_throttle_rows': int((selected['execution_status'] == 'blocked_throttle_halt').sum()), 'blocked_pyramid_rows': int((selected['execution_status'] == 'blocked_pyramid_rule').sum()), 'exit_datetime_fixup_rows': exit_fixup_rows, 'stop_loss_exit_rows': stop_loss_exit_rows, 'trailing_stop_exit_rows': trailing_stop_exit_rows, 'horizon_exit_rows': horizon_exit_rows, 'monthly_obs': int(len(monthly)), 'gross_pnl': gross, 'net_pnl': net, 'roll_cost_total': roll_total, 'total_return_pct': total_return_pct, 'avg_monthly_return_pct': mean_m, 'std_monthly_return_pct': std_m, 'monthly_excess_return_pct': mean_ex, 'std_monthly_excess_return_pct': std_ex, 'monthly_sharpe': sharpe, 'max_drawdown_pct': max_dd, 'annualized_return_pct': ann, 'calmar_like': calmar_like}])
    trade_df = selected.loc[:, [c for c in trade_cols if c in selected.columns]].copy()
    if extra_outputs is not None:
        extra_outputs['throttle_log'] = pd.DataFrame(throttle_log_rows, columns=throttle_cols); extra_outputs['position_lifetime'] = _build_position_lifetime_table(trade_df)
