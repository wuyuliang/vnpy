...
| [trailing_exit.py](trailing_exit.py) | `simulate_trailing_exit()`：基于 ATR 的逐 bar 移动止损模拟，承接可选震荡 taper 事件 |
| [oscillation_taper.py](oscillation_taper.py) | `OscillationUpperBandTaper`：range/compression 中在震荡边界按 linear/stepwise 曲线降仓 |
...
- **oscillation taper 默认 off**：只有 `PortfolioLogicConfig(enable_oscillation_taper=True, ...)`
  且 `cluster|interval` 命中时才会降仓；它写 `position_taper_*` 和
  `exit_reason="oscillation_upper_band_taper"`，不是一个入场 `block_reason`。
